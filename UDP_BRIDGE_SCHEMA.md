# (Legacy) Схема проброса UDP порта - Кастомный UDP Bridge протокол

> ВНИМАНИЕ: Этот документ помечен как Legacy после перехода на `udp2tcp`. Актуальная архитектура описана в `README.md` и `MIGRATION_UDP2TCP.md`. Данный файл сохранён для исторической справки до финального удаления.

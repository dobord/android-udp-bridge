...
static void usage(const char *prog)
{
    ...
    printf(
        "  %s --forward <remote_host> <remote_port> <listen_port> [--forward-hold <sec>] --ssh <host> <user> <pass> "
        "<port>\n",
        prog);
    printf(
        "  %s --udp2tcp <rhost> <rport> <lhost> <lport> <ludp_host> <ludp_port> <rdst_host> <rdst_port> [--stats-wait "
        "<sec>] [--stats-interval <sec>] --ssh <host> <user> <pass> <port>\n",
        prog);
    printf("Options:\n");
    ...
    printf(
        "  --stats-wait <sec>          Total duration to run udp2tcp client before exit (default: 5; 0 = infinite "
        "until Ctrl+C)\n");
    printf("  --stats-interval <sec>      Print stats every <sec> seconds during run (default: disabled)\n");
    printf(
        "  --udp-spam <count> <size> [gap_ms]  Send <count> UDP datagrams of <size> bytes to <ludp_host>:<ludp_port> "
        "while running (default gap=20ms)\n");
}
...
int main(int argc, char **argv)
{
    ...
    const char *ssh_host = nullptr, *ssh_user = nullptr, *ssh_pass = nullptr;
    int ssh_port = 22;
    const char *f_rhost = nullptr;
    int f_rport = 0, f_lport = 0;
    int f_hold = 1;
    const char *u_rhost = nullptr, *u_lhost = nullptr, *u_ludp_host = nullptr, *u_rdst_host = nullptr;
    int u_rport = 0, u_lport = 0, u_ludp_port = 0, u_rdst_port = 0;
    ...
    for (size_t i = 0; i < args.size();) {
        if (args[i] == "--ssh" && i + 4 < args.size()) {
            ssh_host = args[i + 1].c_str();
            ssh_user = args[i + 2].c_str();
            ssh_pass = args[i + 3].c_str();
            ssh_port = std::stoi(args[i + 4]);
            do_ssh = true;
            i += 5;
            continue;
        } else if (args[i] == "--forward" && i + 3 < args.size()) {
            f_rhost = args[i + 1].c_str();
            f_rport = std::stoi(args[i + 2]);
            f_lport = std::stoi(args[i + 3]);
            do_forward = true;
            i += 4;
            continue;
        } else if (args[i] == "--udp2tcp" && i + 8 < args.size()) {
            u_rhost = args[i + 1].c_str();
            u_rport = std::stoi(args[i + 2]);
            u_lhost = args[i + 3].c_str();
            u_lport = std::stoi(args[i + 4]);
            u_ludp_host = args[i + 5].c_str();
            u_ludp_port = std::stoi(args[i + 6]);
            u_rdst_host = args[i + 7].c_str();
            u_rdst_port = std::stoi(args[i + 8]);
            do_udp2tcp = true;
            i += 9;
            continue;
        } else if (args[i] == "--forward-hold" && i + 1 < args.size()) {
            f_hold = std::stoi(args[i + 1]);
            i += 2;
            continue;
        } else if (args[i] == "--udp2tcp-log-level" && i + 1 < args.size()) {
            u_loglevel = args[i + 1].c_str();
            i += 2;
            continue;
        } else if (args[i] == "--stats-wait" && i + 1 < args.size()) {
            u_stats_wait = std::stoi(args[i + 1]);
            i += 2;
            continue;
        } else if (args[i] == "--stats-interval" && i + 1 < args.size()) {
            u_stats_interval = std::stoi(args[i + 1]);
            i += 2;
            continue;
    ...
    if (do_udp2tcp) {
        if (u_loglevel && *u_loglevel) {
            udp2tcp_set_log_level(u_loglevel);
        }
        ...
        rc = ssht_cli_start_udp2tcp(
            ...
            u_lport,
            u_ludp_host ? u_ludp_host : "127.0.0.1",
            u_ludp_port,
            u_rdst_host ? u_rdst_host : "127.0.0.1",
            u_rdst_port);
            ...
            if (u_spam_count > 0 && u_ludp_host && u_ludp_port > 0 && u_spam_size > 0) {
                spammer = std::thread(
                    [&]()
                    {
                        ...
                        sa.sin_port = htons((uint16_t)u_ludp_port);
            ...
            if (u_stats_interval > 0) {
                if (u_stats_wait <= 0) {
                    ...
                    while (true) {
                        ...
                        uint64_t txf = 0, rxf = 0, txb = 0, rxb = 0;
                        udp2tcp_get_library_stats(&txf, &rxf, &txb, &rxb);
                        printf(
                            "stats: TX frames=%llu bytes=%llu, RX frames=%llu bytes=%llu\n",
                            (unsigned long long)txf,
                            (unsigned long long)txb,
                            (unsigned long long)rxf,

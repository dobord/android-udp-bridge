...
static void usage(const char* prog) {
  ...
  printf("  %s --udp2tcp <rhost> <rport> <lhost> <lport> <ludp_host> <ludp_port> <rdst_host> <rdst_port> --ssh <host> <user> <pass> <port>\n", prog);
  printf("Options:\n");
...
int main(int argc, char** argv) {
  ...
  if (do_udp2tcp) {
    ...
    if (rc==0) {
      // Run a short wait then query stats
      fprintf(stdout, "udp2tcp running... press Ctrl+C to stop or wait 5s\n");
      fflush(stdout);

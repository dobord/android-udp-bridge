...
int protocol_create_message(char* buffer, size_t buffer_size, message_type_t type,
    ...
    // Convert to network byte order and copy header
    udp_bridge_header_t net_header = header;
    ...
    memcpy(temp_buffer, &net_header, UDP_BRIDGE_HEADER_SIZE);
    ...
    // Calculate checksum
    uint32_t checksum = protocol_calculate_checksum(temp_buffer, UDP_BRIDGE_HEADER_SIZE + payload_size);
    
    // Update header with checksum
    net_header.checksum = htonl(checksum);
    memcpy(temp_buffer, &net_header, UDP_BRIDGE_HEADER_SIZE);

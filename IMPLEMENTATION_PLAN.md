...
- [ ] Создать директорию `server-udp-bridge/`
- [ ] Настроить Docker окружение
- [ ] Создать Dockerfile с SSH сервером
- [ ] Настроить docker-compose.yml

### 1.2 Реализация протокола
- [ ] Создать `protocol.h` с определениями структур
- [ ] Реализовать `protocol.c` с функциями парсинга
- [ ] Добавить валидацию и checksum
- [ ] Создать unit тесты протокола

### 1.3 Управление клиентами
- [ ] Реализовать `client_table.h/c`
- [ ] Добавить функции добавления/удаления клиентов
- [ ] Реализовать автоматические таймауты
- [ ] Добавить статистику и мониторинг

### 1.4 UDP Forwarder
- [ ] Создать `udp_forwarder.h/c`
- [ ] Реализовать forwarding к целевому серверу
- [ ] Добавить mapping ответов обратно к клиентам
- [ ] Обработка ошибок и reconnect

### 1.5 Main сервер
- [ ] Создать `main.c` с TCP сервером
...
- [ ] Портировать `protocol.h` в Android NDK
- [ ] Создать `udp_bridge_protocol.c`
- [ ] Интегрировать с существующим JNI кодом
- [ ] Добавить протокольные функции

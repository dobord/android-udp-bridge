...
int ssh_pki_import_privkey_file(
    const char *filename, const char *passphrase, void *auth_fn, void *auth_data, ssh_key *pkey);
...
int ssh_channel_open_forward(
    ssh_channel channel, const char *remote_host, int remote_port, const char *sourcehost, int localport);

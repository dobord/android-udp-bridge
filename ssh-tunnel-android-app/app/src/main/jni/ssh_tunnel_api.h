...
// Opaque handle that holds SSH session state and forwarding channels.
// Allocated and returned by connect functions. NULL on error.
typedef struct ssht_handle ssht_handle;

// Connect to SSH server with password authentication.
// Returns 0 on success, non-zero on error.
// Connect to SSH server with password authentication.
// Returns pointer to ssht_handle on success, or NULL on error.
...
// Disconnect given SSH session and stop forwarding. Frees the handle.
void ssht_cli_disconnect(ssht_handle *h);

// Start local TCP port forward (listen_host:listen_port) to remote_host:remote_port via SSH direct-tcpip.
// Returns 0 on success, non-zero on error.
...
// Stop udp2tcp and cleanup for the given handle.
void ssht_cli_stop_udp2tcp(ssht_handle *h);

// Query connection state for the given handle. Returns 1 if connected, 0 otherwise.
int ssht_cli_is_connected(ssht_handle *h);

// Query udp2tcp running state for the given handle. Returns 1 if running, 0 otherwise.
int ssht_cli_is_udp2tcp_running(ssht_handle *h);

// Retrieve udp2tcp stats for the given handle; returns 0 on success.

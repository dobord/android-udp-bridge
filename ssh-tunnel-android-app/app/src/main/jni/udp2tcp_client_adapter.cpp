...
static void udp2tcp_log_cb(int level, const char *message, void * /*user*/)
{
    ...
    switch (level) {
        case 0:
            LOGI("udp2tcp[debug]: %s", message);
            break;
        case 1:
            LOGI("udp2tcp[info]: %s", message);
            break;
        case 2:
            LOGE("udp2tcp[warn]: %s", message);
            break;
        case 3:
            LOGE("udp2tcp[error]: %s", message);
            break;
        default:
            LOGI("udp2tcp[%d]: %s", level, message);
            break;
    }
}
    ...
    int remoteUdpPort)
{
    ...
    (void)remoteBridgeHost;
    (void)remoteBridgePort; // not used directly by adapter; logged upstream
    if (!localBridgeHost || !localUdpHost || !remoteUdpHost)
        return -1;
    if (localBridgePort <= 0 || localUdpPort <= 0 || remoteUdpPort <= 0)
        return -1;
    ...
    LOGI(
        "Starting udp2tcp client: tcp %s:%d, udp-listen %s:%d -> remote %s:%d, log=%s",
        cfg.tcp_connect.host,
        (int)cfg.tcp_connect.port,
        fwd.listen_addr,
        (int)fwd.listen_port,
        fwd.remote_dst_ip,
        (int)fwd.remote_dst_port,
...
int udp2tcp_get_library_stats(uint64_t *tx_frames, uint64_t *rx_frames, uint64_t *tx_bytes, uint64_t *rx_bytes)
{
    ...
    if (udp2tcp_client_get_stats(g_client_handle, &st) != 0)
        return -2;
    if (tx_frames)

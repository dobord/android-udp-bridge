...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if (test_session) { LOGI("JNI_OnLoad: ssh_new() succeeded without init"); ssh_free(test_session);} else { LOGE("JNI_OnLoad: ssh_new() failed without init"); }
...
JNIEXPORT void JNICALL JNI_OnUnload(JavaVM* vm, void* reserved) {
    (void)vm; (void)reserved;  // Suppress unused parameter warnings
    if (g_libssh_initialized) { ssh_finalize(); g_libssh_initialized = 0; }
#ifdef USE_OPENSSL
    if (g_openssl_initialized) { EVP_cleanup(); ERR_free_strings(); g_openssl_initialized = 0; }
#endif
...
void* tcp_port_forward_thread(void* arg) {
    ...
    if (cfg && cfg->remote_host[0] != '\0') { strncpy(remote_host_buf, cfg->remote_host, sizeof(remote_host_buf)-1); remote_host_buf[sizeof(remote_host_buf)-1] = '\0'; }
    else { strncpy(remote_host_buf, "127.0.0.1", sizeof(remote_host_buf)-1); remote_host_buf[sizeof(remote_host_buf)-1] = '\0'; }
    ...
    if (listen_sock < 0) { LOGE("Failed to create listening socket for port forwarding"); return NULL; }
    g_listen_sock = listen_sock;
    int opt = 1; setsockopt(listen_sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    struct sockaddr_in listen_addr; memset(&listen_addr, 0, sizeof(listen_addr));
    listen_addr.sin_family = AF_INET; listen_addr.sin_addr.s_addr = inet_addr("127.0.0.1"); listen_addr.sin_port = htons(listen_port);
    if (bind(listen_sock, (struct sockaddr*)&listen_addr, sizeof(listen_addr)) < 0) {
        LOGE("Failed to bind to port %d for forwarding: %s", listen_port, strerror(errno)); close(listen_sock); return NULL;
    }
    if (listen(listen_sock, 5) < 0) { LOGE("Failed to listen on port %d: %s", listen_port, strerror(errno)); close(listen_sock); return NULL; }
    ...
    while (port_forward_running) {
        ...
        if (client_sock < 0) { if (port_forward_running) { LOGE("Failed to accept connection: %s", strerror(errno)); } continue; }
        ...
        if (session != NULL) {
            ...
            if (channel != NULL) {
                int rc = SSH_ERROR; const int MAX_OPEN_ATTEMPTS = 10; const int RETRY_SLEEP_MS = 200;
                for (int attempt = 1; attempt <= MAX_OPEN_ATTEMPTS; ++attempt) {
                    ...
                    if (rc == SSH_OK) { if (attempt > 1) { LOGI("SSH forward channel opened after %d attempts", attempt);} break; }
                    ...
                    if (err) {
                        ...
                        if (strstr(err, "Connection refused") != NULL || strstr(err, "connection refused") != NULL) {
                            if (attempt < MAX_OPEN_ATTEMPTS) { struct timespec ts; ts.tv_sec = RETRY_SLEEP_MS / 1000; ts.tv_nsec = (RETRY_SLEEP_MS % 1000) * 1000000L; nanosleep(&ts, NULL); continue; }
                        }
                    } else {
                ...
                if (rc == SSH_OK) {
                    ...
                    char buffer[4096]; fd_set read_fds; int session_fd = ssh_get_fd(session);
                    if (session_fd < 0) { LOGW("ssh_get_fd returned <0; forwarding may not progress correctly"); }
                    int max_fd = (client_sock > session_fd) ? client_sock : session_fd; uint64_t last_tick_sec = 0; int connection_active = 1;
                    int flags = fcntl(client_sock, F_GETFL, 0); if (flags >= 0) fcntl(client_sock, F_SETFL, flags | O_NONBLOCK);
                    struct forward_buffer to_ssh; struct forward_buffer to_client; fb_init(&to_ssh); fb_init(&to_client);
                    ...
                    while (port_forward_running && connection_active) {
                        ...
                        FD_ZERO(&read_fds); FD_SET(client_sock, &read_fds); if (session_fd >= 0) { FD_SET(session_fd, &read_fds); }
                        struct timeval tv = {1, 0}; int ready = select(max_fd + 1, &read_fds, NULL, NULL, &tv);
                        if (ready < 0) { if (errno == EINTR) continue; LOGE("select() error in forwarding loop: %s", strerror(errno)); break; }
                        time_t now_sec = time(NULL);
                        if (now_sec != (time_t)last_tick_sec) {
                            last_tick_sec = (uint64_t)now_sec;
                            if (session != NULL && !ssh_is_connected(session)) { LOGW("SSH session no longer connected"); break; }
                        ...
                        if (FD_ISSET(client_sock, &read_fds)) {
                            for (;;) {
                                ...
                                else if (bytes < 0) { if (errno == EWOULDBLOCK || errno == EAGAIN) { break; } LOGE("recv() error: %s", strerror(errno)); connection_active = 0; break; }
                                if (fb_append(&to_ssh, buffer, (size_t)bytes, MAX_BUFFER_CAP) < 0) { LOGE("to_ssh buffer overflow, closing"); connection_active = 0; break; }
                        ...
                        while (port_forward_running && connection_active) {
                            ...
                            if (ssh_bytes > 0) {
                                if (fb_append(&to_client, buffer, (size_t)ssh_bytes, MAX_BUFFER_CAP) < 0) { LOGE("to_client buffer overflow, closing"); connection_active = 0; break; }
                                ...
                                LOGI("SSH channel EOF reached"); connection_active = 0; break;
                            } else if (ssh_bytes == SSH_ERROR) {
                                LOGE("ssh_channel_read_nonblocking error (SSH_ERROR)"); connection_active = 0; break;
                            }
                        }
                        if (to_client.size > to_client.off) fb_flush_to_client(&to_client, client_sock, &connection_active, &port_forward_running);
                        if (!ssh_channel_is_open(channel) || ssh_channel_is_closed(channel)) { LOGI("SSH channel closed by remote side"); break; }
                    }
                    fb_dispose(&to_ssh); fb_dispose(&to_client);
...
int setup_tcp_port_forwarding(const char* remote_host, int remote_port, int listen_port) {
    if (port_forward_thread != 0) { LOGI("TCP port forwarding already running"); return 0; }
    ...
    if (remote_host && *remote_host) { strncpy(cfg->remote_host, remote_host, sizeof(cfg->remote_host)-1); cfg->remote_host[sizeof(cfg->remote_host)-1] = '\0'; }
    else { strncpy(cfg->remote_host, "127.0.0.1", sizeof(cfg->remote_host)-1); cfg->remote_host[sizeof(cfg->remote_host)-1] = '\0'; }
    cfg->remote_port = remote_port > 0 ? remote_port : 8080;
    cfg->listen_port = listen_port > 0 ? listen_port : cfg->remote_port;
    if (pthread_create(&port_forward_thread, NULL, tcp_port_forward_thread, cfg) != 0) { LOGE("Failed to create TCP port forwarding thread"); free(cfg); return -1; }
...
void stop_tcp_port_forwarding() {
    if (port_forward_thread != 0) {
        ...
        port_forward_running = 0; pf_set_state(PF_STATE_SHUTTING_DOWN);
        if (g_listen_sock >= 0) { LOGI("stop_tcp_port_forwarding: closing listen socket %d", g_listen_sock); shutdown(g_listen_sock, SHUT_RDWR); close(g_listen_sock); g_listen_sock = -1; }
        const uint64_t start_ns = monotonic_ns(); int joined = 0;
        ...
        for (int attempt = 0; attempt < 50; ++attempt) { // ~5s max
            ...
            struct timespec slp = {0, 100 * 1000 * 1000}; nanosleep(&slp, NULL);
            if ((attempt % 10) == 0) {
...
#endif
#endif
        if (!joined) { LOGI("stop_tcp_port_forwarding: performing final blocking join (state=%s)", pf_state_str(g_pf_state)); pthread_join(port_forward_thread, NULL); }
        port_forward_thread = 0; uint64_t total_ms = (monotonic_ns() - start_ns)/1000000ull;
...
JNIEXPORT jboolean JNICALL Java_com_example_sshtunnel_SshTunnelService_connectToServer(
    ...
    pthread_mutex_lock(&session_mutex);
    if (session != NULL) { LOGW("Session already exists, disconnecting first"); ssh_disconnect(session); ssh_free(session); session = NULL; }
    session = ssh_new();
    if (session == NULL) { LOGE("Failed to create SSH session"); pthread_mutex_unlock(&session_mutex); (*env)->ReleaseStringUTFChars(env, host, host_str); (*env)->ReleaseStringUTFChars(env, username, username_str); (*env)->ReleaseStringUTFChars(env, password, password_str); return JNI_FALSE; }
    ...
    unsigned char entropy_buf[64]; size_t entropy_len;
    if (android_entropy_source(NULL, entropy_buf, sizeof(entropy_buf), &entropy_len) == 0) {
        LOGI("Pre-connect entropy generation successful: %zu bytes", entropy_len);
        for (int i = 0; i < 3; i++) { unsigned char more_entropy[32]; size_t more_len; if (android_entropy_source(NULL, more_entropy, sizeof(more_entropy), &more_len) == 0) { LOGI("Additional entropy round %d: %zu bytes", i+1, more_len); } }
    } else { LOGE("Critical: Unable to generate entropy before ssh_connect - expect crashes"); }
#else
    ...
    if (connection != SSH_OK) { LOGE("SSH connection failed: %s", ssh_get_error(session)); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); (*env)->ReleaseStringUTFChars(env, host, host_str); (*env)->ReleaseStringUTFChars(env, username, username_str); (*env)->ReleaseStringUTFChars(env, password, password_str); return JNI_FALSE; }
    LOGI("SSH connection established successfully");
    int auth = ssh_userauth_password(session, username_str, password_str);
    if (auth != SSH_AUTH_SUCCESS) { LOGE("SSH authentication failed: %s", ssh_get_error(session)); ssh_disconnect(session); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); (*env)->ReleaseStringUTFChars(env, host, host_str); (*env)->ReleaseStringUTFChars(env, username, username_str); (*env)->ReleaseStringUTFChars(env, password, password_str); return JNI_FALSE; }
...
JNIEXPORT jboolean JNICALL Java_com_example_sshtunnel_SshTunnelService_connectWithKey(
    ...
    if (session != NULL) { LOGW("Session already exists, disconnecting first"); ssh_disconnect(session); ssh_free(session); session = NULL; }
    session = ssh_new();
    if (session == NULL) { LOGE("Failed to create SSH session"); pthread_mutex_unlock(&session_mutex); (*env)->ReleaseStringUTFChars(env, host, host_str); (*env)->ReleaseStringUTFChars(env, username, username_str); (*env)->ReleaseStringUTFChars(env, private_key_path, key_path_str); if (passphrase_str) (*env)->ReleaseStringUTFChars(env, passphrase, passphrase_str); return -1; }
    ssh_options_set(session, SSH_OPTIONS_HOST, host_str);
    ssh_options_set(session, SSH_OPTIONS_PORT, &port);
    ssh_options_set(session, SSH_OPTIONS_USER, username_str);
    ssh_set_blocking(session, 1);
    ...
    ssh_key privkey; int key_result = ssh_pki_import_privkey_file(key_path_str, passphrase_str, NULL, NULL, &privkey);
    if (key_result != SSH_OK) { LOGE("Failed to load private key from %s: %s", key_path_str, ssh_get_error(session)); ssh_disconnect(session); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); (*env)->ReleaseStringUTFChars(env, host, host_str); (*env)->ReleaseStringUTFChars(env, username, username_str); (*env)->ReleaseStringUTFChars(env, private_key_path, key_path_str); if (passphrase_str) (*env)->ReleaseStringUTFChars(env, passphrase, passphrase_str); return -1; }
    int auth = ssh_userauth_publickey(session, username_str, privkey); ssh_key_free(privkey);
    if (auth != SSH_AUTH_SUCCESS) { LOGE("SSH key authentication failed: %s", ssh_get_error(session)); ssh_disconnect(session); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); (*env)->ReleaseStringUTFChars(env, host, host_str); (*env)->ReleaseStringUTFChars(env, username, username_str); (*env)->ReleaseStringUTFChars(env, private_key_path, key_path_str); if (passphrase_str) (*env)->ReleaseStringUTFChars(env, passphrase, passphrase_str); return -1; }
...
JNIEXPORT jboolean JNICALL Java_com_example_sshtunnel_SshTunnelService_connectWithKey(
    JNIEnv *env, jobject obj, jstring host, jint port, jstring username, jstring private_key_path, jstring passphrase) {
    (void)obj; (void)private_key_path; (void)passphrase;
    const char *host_str = (*env)->GetStringUTFChars(env, host, 0);
    const char *username_str = (*env)->GetStringUTFChars(env, username, 0);
...
JNIEXPORT void JNICALL Java_com_example_sshtunnel_SshTunnelService_disconnect(JNIEnv *env, jobject obj) {
    ...
    pthread_mutex_lock(&session_mutex); tunnel_active = 0;
    if (session != NULL) { ssh_disconnect(session); ssh_free(session); session = NULL; LOGI("SSH session disconnected and freed"); }
    else { LOGW("No active SSH session to disconnect"); }
    pthread_mutex_unlock(&session_mutex);
    uint64_t elapsed_ms = (monotonic_ns() - t0)/1000000ull; LOGI("Disconnect sequence finished in %llums", (unsigned long long)elapsed_ms);
}

JNIEXPORT jboolean JNICALL Java_com_example_sshtunnel_SshTunnelService_isConnected(JNIEnv *env, jobject obj) {
    (void)env; (void)obj;
    pthread_mutex_lock(&session_mutex); jboolean connected = (session != NULL) ? JNI_TRUE : JNI_FALSE; pthread_mutex_unlock(&session_mutex);
    return connected;
}

JNIEXPORT jstring JNICALL Java_com_example_sshtunnel_SshTunnelService_nativeDebugDump(JNIEnv *env, jobject obj) {
    (void)obj;
    pthread_mutex_lock(&session_mutex); int session_present = (session != NULL); pthread_mutex_unlock(&session_mutex);
    uint64_t now_ns = monotonic_ns(); uint64_t state_age_ms = (g_pf_last_state_change_mono_ns > 0) ? (now_ns - g_pf_last_state_change_mono_ns)/1000000ull : 0;
    char buf[512];
...
JNIEXPORT jint JNICALL Java_com_example_sshtunnel_SshTunnelService_startUdp2Tcp(
    ...
    if (port_forward_thread == 0) {
        ...
        if (setup_tcp_port_forwarding(eff_remote_bridge_host, eff_remote_bridge_port, local_bridge_port) != 0) { LOGW("Failed to setup TCP port forwarding prior to udp2tcp start"); }
    }
...
JNIEXPORT void JNICALL Java_com_example_sshtunnel_SshTunnelService_stopUdp2Tcp(JNIEnv *env, jobject obj) {
    (void)env; (void)obj;
    if (udp2tcp_is_running()) { LOGI("Stopping udp2tcp adapter"); udp2tcp_stop(); udp2tcp_cleanup(); }
}
...
JNIEXPORT jboolean JNICALL Java_com_example_sshtunnel_SshTunnelService_isUdp2TcpRunning(JNIEnv *env, jobject obj) {
    (void)env; (void)obj; return udp2tcp_is_running() ? JNI_TRUE : JNI_FALSE;
}
#else
...
JNIEXPORT jint JNICALL Java_com_example_sshtunnel_SshTunnelService_startUdp2Tcp(
    ...
    jstring j_local_udp_host,     jint local_udp_port,
    jstring j_remote_udp_host,    jint remote_udp_port) {
    (void)env; (void)obj; (void)j_remote_bridge_host; (void)remote_bridge_port; (void)j_local_bridge_host; (void)local_bridge_port; (void)j_local_udp_host; (void)local_udp_port; (void)j_remote_udp_host; (void)remote_udp_port; LOGW("udp2tcp not enabled in this build (startUdp2Tcp, 8-arg stub)"); return -1;
}

JNIEXPORT void JNICALL Java_com_example_sshtunnel_SshTunnelService_stopUdp2Tcp(JNIEnv *env, jobject obj) { (void)env; (void)obj; LOGW("udp2tcp not enabled in this build (stopUdp2Tcp)"); }
JNIEXPORT jstring JNICALL Java_com_example_sshtunnel_SshTunnelService_getUdp2TcpStats(JNIEnv *env, jobject obj) { (void)obj; return (*env)->NewStringUTF(env, "udp2tcp disabled (build without USE_UDP2TCP)"); }
JNIEXPORT jboolean JNICALL Java_com_example_sshtunnel_SshTunnelService_isUdp2TcpRunning(JNIEnv *env, jobject obj) { (void)env; (void)obj; return JNI_FALSE; }
#endif // USE_UDP2TCP
...
JNIEXPORT jint JNICALL Java_com_example_sshtunnel_SshTunnelService_nativeTlsSelfTest(JNIEnv *env, jobject obj) {
    ...
    if (!ver) { LOGE("nativeTlsSelfTest: OpenSSL_version returned NULL"); return 0; }
    if (strncmp(ver, "OpenSSL", 7) != 0) { LOGE("nativeTlsSelfTest: Unexpected version string: %s", ver); return 0; }
    LOGI("nativeTlsSelfTest: OpenSSL version detected: %s", ver);
    unsigned long vnum = OpenSSL_version_num(); if (vnum == 0) { LOGW("nativeTlsSelfTest: OpenSSL_version_num returned 0"); }
...
int ssht_cli_connect_password(const char* host, int port,
                              const char* username, const char* password) {
    if (!host || !username || !password || port <= 0) { LOGE("ssht_cli_connect_password: invalid arguments"); return -1; }
    signal(SIGPIPE, SIG_IGN);
#ifndef USE_LIBSSH_MOCK
    ssh_set_log_callback(ssh_android_log_cb);
    ...
    pthread_mutex_lock(&session_mutex);
    if (session != NULL) { LOGW("CLI: existing session found, disconnecting"); ssh_disconnect(session); ssh_free(session); session = NULL; }
    session = ssh_new();
    if (!session) { pthread_mutex_unlock(&session_mutex); LOGE("CLI: ssh_new failed"); return -1; }
    ssh_options_set(session, SSH_OPTIONS_HOST, host);
    ssh_options_set(session, SSH_OPTIONS_PORT, &port);
    ssh_options_set(session, SSH_OPTIONS_USER, username);
    ssh_set_blocking(session, 1);
    int rc = ssh_connect(session);
    if (rc != SSH_OK) { LOGE("CLI: ssh_connect failed: %s", ssh_get_error(session)); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); return -2; }
    rc = ssh_userauth_password(session, username, password);
    if (rc != SSH_AUTH_SUCCESS) { LOGE("CLI: password auth failed: %s", ssh_get_error(session)); ssh_disconnect(session); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); return -3; }
    pthread_mutex_unlock(&session_mutex);
    LOGI("CLI: SSH connection established");
    return 0;
}

int ssht_cli_connect_key(const char* host, int port, const char* username,
                         const char* private_key_path, const char* passphrase) {
    if (!host || !username || !private_key_path || port <= 0) { LOGE("ssht_cli_connect_key: invalid arguments"); return -1; }
    signal(SIGPIPE, SIG_IGN);
#ifndef USE_LIBSSH_MOCK
    ssh_set_log_callback(ssh_android_log_cb);
    ssh_set_log_level(SSH_LOG_PROTOCOL);
    ...
    if (rc != SSH_OK) { LOGE("CLI: ssh_connect failed: %s", ssh_get_error(session)); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); return -2; }
#ifndef USE_LIBSSH_MOCK
    ssh_key privkey; int key_result = ssh_pki_import_privkey_file(private_key_path, passphrase, NULL, NULL, &privkey);
    if (key_result != SSH_OK) { LOGE("CLI: failed to load private key from %s: %s", private_key_path, ssh_get_error(session)); ssh_disconnect(session); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); return -3; }
    rc = ssh_userauth_publickey(session, username, privkey); ssh_key_free(privkey);
    if (rc != SSH_AUTH_SUCCESS) { LOGE("CLI: key auth failed: %s", ssh_get_error(session)); ssh_disconnect(session); ssh_free(session); session = NULL; pthread_mutex_unlock(&session_mutex); return -4; }
#else
    (void)private_key_path; (void)passphrase; rc = SSH_AUTH_SUCCESS;
#endif
    pthread_mutex_unlock(&session_mutex);
...
void ssht_cli_disconnect(void) {
    ...
    stop_tcp_port_forwarding();
    pthread_mutex_lock(&session_mutex);
    if (session) { ssh_disconnect(session); ssh_free(session); session = NULL; LOGI("CLI: session disconnected"); }
    pthread_mutex_unlock(&session_mutex);
}

int ssht_cli_start_port_forward(const char* remote_host, int remote_port, int listen_port) {
    if (!remote_host || remote_port <= 0 || listen_port <= 0) { LOGE("CLI: start_port_forward invalid args"); return -1; }
    return setup_tcp_port_forwarding(remote_host, remote_port, listen_port);
}

int ssht_cli_start_udp2tcp(const char* remote_bridge_host, int remote_bridge_port,
                           const char* local_bridge_host,  int local_bridge_port,
                           const char* local_udp_host,     int local_udp_port,
                           const char* remote_udp_host,    int remote_udp_port) {
#ifdef USE_UDP2TCP
    if (port_forward_thread == 0) {
    ...
    return udp2tcp_start(
        ...
        remote_udp_port);
#else
    (void)remote_bridge_host; (void)remote_bridge_port; (void)local_bridge_host; (void)local_bridge_port; (void)local_udp_host; (void)local_udp_port; (void)remote_udp_host; (void)remote_udp_port;
...
#ifdef USE_UDP2TCP
    if (udp2tcp_is_running()) { udp2tcp_stop(); udp2tcp_cleanup(); }
#endif
}

int ssht_cli_is_connected(void) {
    pthread_mutex_lock(&session_mutex); int ok = (session != NULL) ? 1 : 0; pthread_mutex_unlock(&session_mutex); return ok;
}
...
int ssht_cli_get_udp2tcp_stats(uint64_t* tx_frames, uint64_t* rx_frames,
                               uint64_t* tx_bytes,  uint64_t* rx_bytes) {
#ifdef USE_UDP2TCP
    if (!tx_frames || !rx_frames || !tx_bytes || !rx_bytes) return -1;
    udp2tcp_get_library_stats(tx_frames, rx_frames, tx_bytes, rx_bytes);
    return 0;
#else
    (void)tx_frames; (void)rx_frames; (void)tx_bytes; (void)rx_bytes; return -1;
#endif
}

...
# Specify the source files (udp2tcp path). Legacy sources (udp_bridge_protocol.c, udp_listener.c, client_manager.c, tcp_connection_manager.c) будут постепенно удалены.
...
# Включаем режим udp2tcp
LOCAL_CFLAGS += -DUSE_UDP2TCP
LOCAL_CPPFLAGS += -std=c++17 -DUSE_UDP2TCP
LOCAL_C_INCLUDES += $(LOCAL_PATH)/../../../../third_party/udp2tcp/include

# Legacy (временно оставлено для совместимости, можно закомментировать, если не нужен старый функционал)
...
# Legacy вторичная библиотека отключена (раскомментируйте для старого режима)
# include $(CLEAR_VARS)

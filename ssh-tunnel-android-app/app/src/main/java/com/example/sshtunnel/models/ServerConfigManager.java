...
public class ServerConfigManager {
        ...
        if (config.getId() != 0) {
            for (ServerConfig existingConfig : configs) {
                if (existingConfig.getId() == config.getId()) {
                    android.util.Log.w("ServerConfigManager", "Attempted to add config that already exists with ID: "
                            + config.getId() + ". Use updateConfig instead.");
                    return;
                }
            }
        }
    ...
    public void updateConfig(ServerConfig config) {
        ...
        for (int i = 0; i < configs.size(); i++) {
            android.util.Log.d("ServerConfigManager",
                    "Checking config at index " + i + " with ID: " + configs.get(i).getId());
